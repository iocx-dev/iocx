...
int main(void) {
    FILE *f = fopen("load_config_zero_size_invalid_rva.full.exe", "wb");
    if (!f) return 1;
    ...
    FILE_HDR fh = {0};
    fh.Machine = 0x8664; fh.NumberOfSections = 2;
    fh.SizeOfOptionalHeader = sizeof(OPT64); fh.Characteristics = 0x22;
    w(f, &fh, sizeof(fh));

    OPT64 opt = {0};
    opt.Magic = 0x20B; opt.AddressOfEntryPoint = 0x1000;
    opt.BaseOfCode = 0x1000; opt.ImageBase = 0x140000000ULL;
    opt.SectionAlignment = 0x1000; opt.FileAlignment = 0x200;
    opt.SizeOfImage = 0x4000; opt.SizeOfHeaders = 0x400;
    opt.Subsystem = 3; opt.NumDirs = 16;

    opt.DataDir[10].VirtualAddress = 0x90000000; // unmapped
    opt.DataDir[10].Size = 0; // zero-length
    w(f, &opt, sizeof(opt));
