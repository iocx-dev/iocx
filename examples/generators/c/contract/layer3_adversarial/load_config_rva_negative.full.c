...
int main(void){
    FILE *f=fopen("load_config_rva_negative.full.exe","wb");
    if(!f)return 1;
    ...
    opt.DataDir[10].VirtualAddress=0xFFFFFFFF; // bogus high RVA
    ...
    pad(f,0x600); LOAD_CONFIG64 lc={0}; lc.Size=sizeof(LOAD_CONFIG64); w(f,&lc,sizeof(lc));
