...
int main(void){
    FILE *f=fopen("directory_zero_size_nonzero_rva.full.exe","wb");
    if(!f) return 1;
    ...
    FILE_HDR fh={0};
    fh.Machine=0x8664;
    fh.NumberOfSections=2; /* .text + .zero */
    ...
    opt.DataDir[1].VirtualAddress = 0x9000; /* outside any section */
    opt.DataDir[1].Size = 0x200;

    opt.DataDir[2].VirtualAddress = 0x2000; /* inside zero-length section */
    opt.DataDir[2].Size = 0x100;

    opt.DataDir[10].VirtualAddress = 0x1000;
    opt.DataDir[10].Size = 0;
    ...
    /* ZERO-LENGTH SECTION */
    SECT zero={0};
    memcpy(zero.Name,".zero",5);
    zero.VirtualSize=0; /* zero-length region */
    zero.VirtualAddress=0x2000;
    zero.SizeOfRawData=0;
    zero.PointerToRawData=0; /* unmapped */
    zero.Characteristics=0x40000040;
    w(f,&zero,sizeof(zero));
