...
int main(void){
    FILE *f=fopen("directory_raw_mismatch.full.exe","wb");
    if(!f) return 1;
    ...
    FILE_HDR fh={0};
    fh.Machine=0x8664;
    fh.NumberOfSections=1; /* only .text */
    ...
    /* RAW MISMATCH:
       RVA=0x1800 maps into .text (0x1000–0x2000)
       raw_offset = 0x200 + (0x1800 - 0x1000) = 0xA00 (outside raw range)
    */
    opt.DataDir[3].VirtualAddress = 0x1800;
    opt.DataDir[3].Size = 0x100;
    ...
    text.SizeOfRawData=0x200; /* raw range: 0x200–0x400 */
