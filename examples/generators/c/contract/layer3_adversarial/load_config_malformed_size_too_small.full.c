...
int main(void){
    FILE *f=fopen("load_config_malformed_size_too_small.full.exe","wb");
    if(!f)return 1;

    DOS dos={0}; dos.e_magic=0x5A4D; dos.e_lfanew=0x80;
    w(f,&dos,sizeof(dos)); pad(f,dos.e_lfanew);

    PE_SIG sig={0x00004550}; w(f,&sig,sizeof(sig));

    FILE_HDR fh={0}; fh.Machine=0x8664; fh.NumberOfSections=2;
    fh.SizeOfOptionalHeader=sizeof(OPT64); fh.Characteristics=0x22;
    w(f,&fh,sizeof(fh));

    OPT64 opt={0};
    opt.Magic=0x20B; opt.AddressOfEntryPoint=0x1000; opt.BaseOfCode=0x1000;
    opt.ImageBase=0x140000000ULL; opt.SectionAlignment=0x1000;
    opt.FileAlignment=0x200; opt.SizeOfImage=0x4000; opt.SizeOfHeaders=0x400;
    opt.Subsystem=3; opt.NumDirs=16;

    // TOO SMALL: directory size < minimum (0x70)
    opt.DataDir[10].VirtualAddress=0x3000;
    opt.DataDir[10].Size=0x20;
    ...
    rdata.SizeOfRawData=0x80; rdata.PointerToRawData=0x600;
    ...
    pad(f,0x600);
    LOAD_CONFIG64 lc={0};
    lc.Size=0x70; lc.SecurityCookie=0x3500;

    // FULL WRITE → NOT truncated
    w(f,&lc,sizeof(lc));
